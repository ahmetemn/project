...
const SubBranchPhotoList = ({ selectBranchesBranchesID  ,buttonClicked }) => {
    ...
    useEffect(() => {
        ...
        try {
            fetch("http://localhost:3002/hastane/hastaneBranchesPhoto",
                {
                    method: "POST",
                    body: JSON.stringify({ userId: user, branches_id: selectBranchesBranchesID }),
                    headers: {
                        Authorization: `Bearer ${accessToken}`,
                        "Content-type": "application/json; charset=UTF-8"
                    },
    ...
    }, [selectBranchesBranchesID,buttonClicked,deletePhoto])
...
export default SubBranchPhotoList;